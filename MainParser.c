...
ez_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern ez_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern ez_int line; /* source code line number - defined in scanner.c */
...
extern ez_void startParser(ez_void);
extern ez_int startScanner(BufferPointer sc_buf);

static ez_void printParserError(ez_string fmt, ...);
static ez_void displayParser(BufferPointer ptrBuffer);
static ez_long getParserFileSize(ez_string fname);
static ez_void callGarbageCollector(ez_void);
...
ez_int mainParser(ez_int argc, ez_string* argv) {

	numParserErrors = 0;			/* Initializes the errors */

	FILE* fi;       /* input file handle */
	ez_int loadsize = 0; /*the size of the file loaded in the buffer */
...
ez_void printParserError(ez_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(ez_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
ez_long getParserFileSize(ez_string fname) {
	FILE* input;
	ez_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
ez_void displayParser(BufferPointer ptrBuffer) {
...
ez_void callGarbageCollector(ez_void) {
